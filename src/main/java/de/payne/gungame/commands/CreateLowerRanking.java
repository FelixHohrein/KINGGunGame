...
public class CreateLowerRanking implements CommandExecutor, TabCompleter {

	@Override
	public final boolean onCommand(final CommandSender commandSender, final Command command, final String commandLabel,
		...
		if (!player.hasPermission("gungame.admin.createlowerranking")) {
		...
		GunGame.getInstance().getSettings().setTopLowerPlayersPosition(player.getLocation());
		player.sendMessage(GunGame.getInstance().getTextEngine().getMessage(MESSAGE.COMMAND_CREATELOWERRANKING_SUCCESS, player.getUniqueId(), true));
